...
class TensorDeserializer(
    ...
    def _bulk_load(
        ...
        if self._plaid_mode:
            buffer_ids = {k: self._buffer_ids[k] for k in keys}
            # Throttling is based on the number of distinct buffers involved
            # based on the set of keys being requested. In a future revision,
            # the buffers should be chosen within this function, so they can
            # always be placed for maximal efficiency.
            # Round-robin scheduling doesn't exactly work here because the order
            # of the list of keys being requested doesn't have to correspond to
            # the order that the buffers were originally assigned. Instead,
            # atomic countdowns need to stick with whichever buffer is
            # associated with each key being loaded.
            # Arbitrarily associate buffer IDs with countdowns
            distinct_buffer_ids = set(buffer_ids.values())
            countdowns = {
                buffer_id: atomic_countdown(tasks_per_tensor, 0)
                for buffer_id in distinct_buffer_ids
            }
            # Ensure each atomic countdown sticks with the same buffer
            countdown_cycle = tuple(countdowns[buffer_ids[k]] for k in keys)
        else:
        ...
        def ready_buffers(
            ...
            # This function was originally for yielding buffers on-demand.
            # In its current state, it primes buffers using madvise before they
            # are used, and returns their names. It is a good candidate to adapt
            # for round-robin buffer cycling for plaid mode in the future.
            can_madvise = (
                self._buffers
            ...
            # start = 0
            # pagesize_mask = ~(mmap.PAGESIZE - 1)
            for buf_name in key_list:
                self._buffers[buf_name].madvise(mmap.MADV_WILLNEED)
                # Other unused code for madvise on a contiguous mmap buffer
                #
                # size = self._metadata[buf_name].data_length
                # end = start + size
                # region_start = start & pagesize_mask
                # region_length = end - region_start
                # mem.madvise(mmap.MADV_WILLNEED, region_start, region_length)
                # start = end
        ...
        try:
            ...
            if computation_threads is not None:
                ...
                # v_hash_start = time.monotonic()
                computation_threads.shutdown(wait=True)
                # v_hash_end = time.monotonic()
                # v_hash_time = v_hash_end - v_hash_start
                # print(f"{v_hash_time:.2f} seconds spent on validation")
...
class TensorSerializer:
    ...
    #
    # def _bulk_writer(self, tensors: Iterable[_WriteSpec]):
