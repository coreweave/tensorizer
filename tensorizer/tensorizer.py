...
class GooseTensorizer:
    ...
    def __init__(
            self,
            file_obj: Union[io.BufferedIOBase, io.RawIOBase, str],
            compress_tensors=False) -> None:
    ...
    def state_dict(
        ...
        """
        Load the tensors in this GooseTensorizer object into a state_dict.
    ...
    def write_state_dict(self, state_dict: Dict):
        """
        Write the state_dict to the file.
        """
        idx = 0
...
def serialize_model(
    ...
    for module_name, module in model.named_modules():
        for name, param in module.named_parameters(recurse=False):
            ...
            ts.write_tensor(idx, module_name + "." + name, TensorType.PARAM, v)
            setattr(module, name, None)
        for name, buf in module.named_buffers(recurse=False):
            v = buf.cpu().detach().numpy()
            ts.write_tensor(idx, module_name + "." + name, TensorType.BUFFER, v)
            setattr(module, name, None)
        idx += 1
