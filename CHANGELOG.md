...
- `TensorDeserializer` now skips complex `dtype` tensors when casting
  to real `dtype`s with `TensorDeserializer(..., dtype=...)`
  - Previously, passing a real `dtype` (e.g. `torch.bfloat16`) to
    `TensorDeserializer` would cast only skip casting for `bool` tensors,
    but not complex ones, which dropped the imaginary parts of complex tensors
  - Complex-to-complex casts (e.g. `torch.complex64` to `torch.complex128`)
    are unaffected
...
[Unreleased]: https://github.com/coreweave/tensorizer/compare/v2.12.1...HEAD
